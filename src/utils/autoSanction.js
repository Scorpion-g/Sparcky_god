...
async function checkAndSanction(member, warnCount) {
  ...
  try {
    const logChannel = member.guild.channels.cache.get(guildConfig?.modLogChannel);
    ...
    if (action === "timeout") {
      if (member.guild.members.me.permissions.has(PermissionFlagsBits.ModerateMembers)) {
        ...
        const embed = new EmbedBuilder()
          .setColor("#FFFF00")
          .setDescription(`⏲️ <@${member.id}> a été mis en timeout automatiquement pour avoir atteint ${warnCount} warns.`)
          .setTimestamp();

        if (logChannel && logChannel.isTextBased()) logChannel.send({ embeds: [embed] });
      }
    } else if (action === "kick") {
      if (member.kickable) await member.kick(reasonText);

      const embed = new EmbedBuilder()
        .setColor("#FFA500")
        .setDescription(`🔨 <@${member.id}> a été expulsé automatiquement pour avoir atteint ${warnCount} warns.`)
        .setTimestamp();

      if (logChannel && logChannel.isTextBased()) logChannel.send({ embeds: [embed] });
    } else if (action === "ban") {
      if (member.bannable) await member.ban({ reason: reasonText });

      const embed = new EmbedBuilder()
        .setColor("#FF0000")
        .setDescription(`🔨 <@${member.id}> a été banni automatiquement pour avoir atteint ${warnCount} warns.`)
        .setTimestamp();

      if (logChannel && logChannel.isTextBased()) logChannel.send({ embeds: [embed] });
    }
  } catch (error) {
    console.error(`Erreur lors de la sanction automatique de ${member.user.tag}:`, error);
  }
}
