...
module.exports = {
    ...
    callback: async (client, interaction) =>{
        ...
        if (memberRolePosition >= requestUserRolePosition){
            await interaction.editReply("Vous ne pouvez pas kick ce membre car il a un rôle superieur ou égale  à vous");
            return;
        }
        if( memberRolePosition >= botRolePosition){
            await interaction.editReply("je ne peux pas kick ce membre car il a un rôle superieur ou égale a vous");
            return;
        }
        try {
